...
LOG_MODULE_REGISTER(cert_gatt, LOG_LEVEL_DBG);

#define DEVICE_CERT_WINDOW_SIZE 4
                                     ...
                                     uint16_t length)
{
    ...
    if (NULL == data)
    {
        LOG_DBG("Subscription terminated, free params");
        free(params);
        return BT_GATT_ITER_STOP;
    }
...
static void gateway_server_cert_write_start(struct bt_conn *conn)
{
    ...
    struct bt_gatt_subscribe_params *subscribe_params =
        malloc(sizeof(struct bt_gatt_subscribe_params));
...
static void gateway_device_cert_read_start(struct bt_conn *conn)
{
    ...
    node->device_cert_receiver = pouch_gatt_receiver_create(send_ack_cb,
                                                            conn,
                                                            device_cert_data_received_cb,
                                                            conn,
                                                            DEVICE_CERT_WINDOW_SIZE);
